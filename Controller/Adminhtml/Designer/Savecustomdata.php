...
class Savecustomdata extends \Magento\Framework\App\Action\Action
{
	...
	public function execute()
	{
		...
		if (empty($errors)) {
			$filestoUpload = $this->getRequest()->getFiles('thumbimage');
			if (!empty($filestoUpload['name'])) {
				...
				if (empty($errors)) {
					...
					$tmpfilepath = $this->_filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath()."customizeproducts/tmp/".$file_name;
