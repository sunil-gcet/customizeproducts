...
class Edit extends \Magento\Backend\Block\Widget\Form\Container
{
    protected function _construct()
    {
        ...
        $this->_controller = 'adminhtml_customizeprodview';

        parent::_construct();

        $this->buttonList->update('save', 'label', __('Save View'));
        $this->buttonList->update('delete', 'label', __('Delete View'));
    ...
    public function getHeaderText()
    {
        if ($this->_coreRegistry->registry('checkmodule_checkmodel')->getId()) {
            return __("Edit View '%1'", $this->escapeHtml($this->_coreRegistry->registry('checkmodule_checkmodel')->getTitle()));
        } else {
            return __('New View');
        }
    }
