...
var fpd_admin_opts = {"adminAjaxUrl":ADMIN_LAYER_AJAX,"upload_image":ADMIN_LAYER_UPLOADIMAGE,"checkToken":"1f6b4acd3f4fcae8e1854e43b1358636","enterTitlePrompt":"Please enter a title","tryAgain":"Something went wrong. Please try again!","addToLibrary":"Add imported image source to media library?","remove":"Are you sure you want to delete it?."};
var fpd_product_builder_opts = {"adminUrl":ADMIN_LAYER_AJAX,"upload_image":ADMIN_LAYER_UPLOADIMAGE,"originX":"center","originY":"center","paddingControl":10,"defaultFont":"Arial","enterTitlePrompt":"Enter a title for the element","chooseElementImageTitle":"Choose an element image","set":"Set","enterYourText":"Enter your text.","removeElement":"Remove element?","notChanged":"You have not saved your changes!","changeImageSource":"Change Image Source"};
var host_mode = false;
	...
	$('#fpd-layers-container .nav.nav-tabs li').click(function(evt) {
		evt.preventDefault();
	...
	$('#fpd-view-switcher').change(function() {
		var $this = $(this);
		var alteredURL = $('#fpd_site_url').val() + 'view_id/'+$this.val()+"/";
		$('#fpd-submit').attr('action', alteredURL).submit();
        window.parent.location = alteredURL;
	});
	...
	$('#fpd-add-image-element, #fpd-add-text-element, #fpd-add-curved-text-element, #fpd-add-upload-zone').click(function(evt) {
		...
		if(this.id == 'fpd-add-image-element' || this.id == 'fpd-add-upload-zone') {
            ...
            $('#fpd-add-image-element-image').click();

			// mediaUploader.elementTitle = elementTitle;

            $('#fpd-add-image-element-image').unbind('change').bind('change', function (e) {
                    ...
                    $.ajax({
                            url: fpd_admin_opts.upload_image, // point to server-side PHP script 
                            ...
                            success: function(response){
								if (typeof response.newpath == 'undefined') {
                                    alert(fpd_admin_opts.tryAgain);
                                } else {
                                    if (typeof response.error !== 'undefined' && response.error.length > 0)
                                    {    
                                        $.each(response.error, function(index, error) {
                                            alert(response.name + ' : ' + error);
                                        });
                                    }    
                                    else        
                                    {    
                                        var imageParams = {index: response.name};
                                        if(addUploadZone) {
	...
	$elementLists.on('click', 'li', function(evt) {
        if (!$(this).hasClass("activetab")) {
            $elementLists.find('li').removeClass('activetab');
            ...
            $.ajax({
                    ...
                    success: function(response){
                        if (typeof response.file == 'undefined') {
                            ...
                            if (typeof response.file.error !== 'undefined' && response.file.error.length > 0)
                            {    
                                $.each(response.file.error, function(index, error) {
                                    alert(response.file.name + ' : ' + error);
                                });
                            }    
                            else        
                            {    
                                var imageParams = {index: response.file.index};
                                $listItem.find('.fpd-element-identifier img').attr('src', response.file.url);
                                $listItem.find('[name="element_sources[]"]').val(response.file.url);
                            }
                        }
                    }
            });
	...
	$('input[name="bounding_box_control"]').change(function() {
		...
		if($(this).is(':checked')) {
			...
			$('.bounding_box_by_other').removeClass('hidden');
            $('input[name="bounding_box_by_other"]').val('');
		}
		else {
			$('#boundig-box-params').show();
            $('#boundig-box-params input').val('');
			$('.bounding_box_by_other').addClass('hidden');
            $('input[name="bounding_box_by_other"]').val('');
		}
		...
		if(currentElement) {
			...
			var copyParams = _parameterStringToObject($currentListItem.find('[name="element_parameters[]"]').val()),
				newParams = $.extend({}, copyParams, {
					...
					index: $elementLists.children('li').length,
	...
	function _setFormFields(element) {
		...
		if(paramsObject.bounding_box_control == '1') {
			...
			if(paramsObject.bounding_box_by_other) {
                ...
                $('.bounding_box_by_other').removeClass('hidden');
				$('input[name="bounding_box_by_other"]').val(paramsObject.bounding_box_by_other);
				if(paramsObject.bounding_box_by_other) {
		...
		if(paramsObject.uploadZone == '1') {
			...
			$('.nav.nav-tabs a[href="#bb-options"]').hide();
			$('.nav.nav-tabs a[href="#upload-zone-options"]').show();
			...
			$('.nav.nav-tabs a[href="#bb-options"]').show();
			$('.nav.nav-tabs a[href="#upload-zone-options"]').hide();
		}
...
function removeParam(key, sourceURL) {
    ...
    var rtn = sourceURL.replace( regex ,'');
    return rtn;
}
