		...
		if ($data) {
			...
			if (isset($action)) {
				switch ($action) {
					...
					case 'fpd_newproduct':
						$results = $this->_ajaxHelper->new_product($data);
						break;

					case 'fpd_editproduct':
						$results = $this->_ajaxHelper->edit_product($data);
						break;

					case 'fpd_removeproduct':
						$results = $this->_ajaxHelper->remove_product($data);
						break;

					case 'fpd_newview':
						$results = $this->_ajaxHelper->new_view($data);
						break;
					...
					case 'fpd_loadorder':
						$results = $this->_ajaxHelper->load_order($data);
						break;

					case 'fpd_loadorderitemimages':
						$results = $this->_ajaxHelper->load_order_item_images($data);
						break;

					case 'fpd_pdffromdataurl':
						$results = $this->_ajaxHelper->create_pdf_from_dataurl($data, $this->getRequest()->getFiles());
						break;

					case 'fpd_imagefromsvg':
						$results = $this->_ajaxHelper->create_image_from_svg($data);
						break;

					case 'fpd_imagefromdataurl':
						$results = $this->_ajaxHelper->create_image_from_dataurl($data, $this->getRequest()->getFiles());
						break;
				}
